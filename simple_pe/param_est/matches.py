...
def _neg_wf_match(x, x_directions, data, f_low, psd, approximant,
                  fixed_pars=None):
    """
    ...
    :param x_directions: list of parameters for which to calculate
        waveform variations
    ...
    m = match(data, h, psd, low_frequency_cutoff=f_low,
              subsample_interpolation=True)[0]

    return -m


def _log_wf_mismatch(x, x_directions, data, f_low, psd, approximant,
                     fixed_pars=None):
    """
    Calculate the negative waveform match, taking x as the values and
    dx as the parameters.  This is in a format that's appropriate
...
def find_best_match(data, x, dx_directions, f_low, psd,
    ...
    :param dx_directions: list of parameters for which to calculate
        waveform variations
    :param f_low: low frequency cutoff
    :param psd: the power spectrum to use in calculating the match
    :param approximant: the approximant to use
    ...
    if (method != 'metric') and (method != 'scipy'):
        ...
        bounds = parameter_bounds.param_bounds(x, dx_directions, harm2=False)
        ...
        out = optimize.minimize(_log_wf_mismatch, x0,
                                args=(dx_directions, data, f_low, psd,
                                      approximant, fixed_pars),
                                bounds=bounds)
        ...
        g = metric.Metric(x, dx_directions, mismatch, f_low, psd,
                          approximant, tolerance)
        g.iteratively_update_metric()

        while True:
            h = waveform.make_waveform(x, g.psd.delta_f, g.f_low,
                                       len(g.psd), g.approximant)
            ...
            for i in range(g.ndim):
                for j in range(2):
                    ...
                    h = waveform.make_offset_waveform(x, g.normalized_evecs()[i:i + 1], alphas[i, j] * (-1) ** j,
                                             ...
                                             g.approximant)
                    matches[i, j] = match(data, h, g.psd,
                                          low_frequency_cutoff=g.f_low,
                                          subsample_interpolation=True)[0]
        ...
        delta_x = pe.SimplePESamples(SamplesDict(dx_directions,
                                                 np.matmul(g.normalized_evecs().samples, s)))
        alpha = waveform.check_physical(x, delta_x, 1)

        h = waveform.make_offset_waveform(x, delta_x, alpha, psd.delta_f,
                                          f_low, len(psd), approximant)
