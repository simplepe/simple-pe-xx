...
data = {}
data[
    "S6 H1"] = "/Users/stephenfairhurst/papers/cgp_fairhurst/ligo/localization/data/lho4k_15May2010_05hrs17min45secUTC_strain.txt"
...
def read_asd(name):
    if name in data.keys():
        freq, amp_spec = np.loadtxt(data[name], unpack=True)
...
def calculate_moments(freq, amp, psd, name=None):
    J_0 = np.trapz(amp ** 2 / psd, freq)
    J_1 = np.trapz(freq * amp ** 2 / psd, freq)
    J_2 = np.trapz(freq ** 2 * amp ** 2 / psd, freq)
    if name:
...
def calculate_max_med(freq, amp, psd):
    ...
    j_0 /= j_0[-1]
    return (freq[np.argmax(amp ** 2 / psd)], min(freq[j_0 > 0.5]))


def mchirp_moments(freq, amp, psd):
    J_0 = np.trapz(amp ** 2 / psd, freq)
    J_1 = np.trapz(freq ** (-5. / 3) * amp ** 2 / psd, freq)
    J_2 = np.trapz(freq ** (-10. / 3) * amp ** 2 / psd, freq)
    snrsq = 4 * J_0
    f53_mean = J_1 / J_0
...
def calculate_higher_moments(freq, amp, psd):
    ...
    J_1 = np.trapz(freq * amp ** 2 / psd, freq)
    J_2 = np.trapz(freq ** 2 * amp ** 2 / psd, freq)
    J_3 = np.trapz(freq ** 3 * amp ** 2 / psd, freq)
    J_4 = np.trapz(freq ** 4 * amp ** 2 / psd, freq)
...
def chirp_moments(freq, psd):
    J = {}
    J7 = np.trapz(freq ** (-7. / 3) / psd, freq)
    for x in [-17, -12, -9, -4, -1]:
        J[x] = np.trapz(freq ** (x / 3.) / psd, freq) / J7
    return J
...
def all_moments(freq, psd, fcut=None):
    if fcut:
        ...
        p = psd
    J = {}
    J7 = np.trapz(f ** (-7. / 3) / p, f)
    for n in range(1, 18):
        J[n] = np.trapz(f ** (-n / 3.) / p, f) / J7
    return J
